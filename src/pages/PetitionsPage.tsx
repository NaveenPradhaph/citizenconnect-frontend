...
const PetitionsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPetitions = async () => {
      setIsLoading(true);
      try {
        let url = "https://citizenconnect-backend.vercel.app/api/petitions";
        if (role == "government") {
          url = `https://citizenconnect-backend.vercel.app/api/petitions?department=${department}`;
        }
  ...
  const applyFilters = (filters: any) => {
    ...
    if (filters.status.length > 0) {
      filtered = filtered.filter((petition) =>
        filters.status.includes(petition.status)
      );
    }
    ...
    if (filters.category.length > 0) {
      filtered = filtered.filter((petition) =>
        filters.category.includes(petition.category)
      );
    }
    ...
    if (filters.governmentLevel.length > 0) {
      filtered = filtered.filter((petition) =>
        filters.governmentLevel.includes(petition.governmentLevel)
      );
    }
    ...
    if (filters.priority.length > 0) {
      filtered = filtered.filter((petition) =>
        filters.priority.includes(petition.priority)
      );
    }
